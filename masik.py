...
def iteration(i):
	...
	for demand in DATA["simulation"]["demands"]:
		if demand["start-time"] == i + 1:
			...
			routes = possible_circuits(DATA["possible-circuits"],
			                           demand["end-points"][0], demand["end-points"][1])
...
def draw():
	...
	edge_has_demand = [
	    (u, v) for (u, v, d) in GRAPH.edges(data=True) if "demands" in d
	]
	edge_has_no_demand = [
	    (u, v) for (u, v, d) in GRAPH.edges(data=True) if not "demands" in d
	]
	...
	for edge in edge_has_demand:
		...
		hex_color = '#' + str(int(round(color[0] * 64))).zfill(2) + str(
		    int(round(color[1] * 64))).zfill(2) + str(int(round(
		        color[2] * 64))).zfill(2)
		colors_has_demand.append(hex_color)
		widths_has_demand.append(
		    transform(GRAPH.edges[edge]["capacity"] - rem, 0,
		              GRAPH.edges[edge]["capacity"], 8, 18))
	...
	nx.draw_networkx_edges(
	    GRAPH,
	    POS,
	    edgelist=edge_has_demand,
	    width=widths_has_demand,
	    edge_color=colors_has_demand)
	nx.draw_networkx_edges(
	    GRAPH,
	    POS,
	    edgelist=edge_has_no_demand,
	    width=widths_has_no_demand,
	    alpha=0.6)
	# labels
	nx.draw_networkx_labels(
	    GRAPH, POS, font_size=20, font_family='sans-serif', alpha=0.9)
...
def possible_circuits(circuits, point_a, point_b):
	...
	for circuit in circuits:
		if (circuit[0] == point_a and circuit[len(circuit) - 1] == point_b) or (
		    circuit[0] == point_b and circuit[len(circuit) - 1] == point_a):
		...
		if not index == 0:
			...
			if available_bandwith - demand["demand"] < 0:
				...
				if not test:
					...
					print ("\t\tedges along the route appended with the demand: from: " + str(
					    route[index - 1]) + " to: " + str(val) + " edge: " + str(edge))
	return valid


with open("cs1.json", "r") as _file:
	...
	for link in DATA["links"]:
		GRAPH.add_edge(
		    link["points"][0], link["points"][1], capacity=link["capacity"])
